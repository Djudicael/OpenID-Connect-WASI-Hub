...
impl GroupRepo {
    ...
    /// If realm_id is None, returns groups from all realms.
    pub async fn list(
        &self,
        conn: &mut Connection,
    ...
    /// Count groups in a realm. If realm_id is None, counts all realms.
    pub async fn count(
        &self,
        conn: &mut Connection,
        realm_id: Option<Uuid>,
        ...
        let sql = match realm_id {
            Some(_) => "SELECT COUNT(*) FROM groups WHERE deleted_at IS NULL AND realm_id = $1",
