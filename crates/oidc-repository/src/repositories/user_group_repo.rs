...
use oidc_core::models::{Group, Role, User};
use uuid::Uuid;

/// Column list for group SELECT queries (order must match GroupRepo::map_row indices).
const GROUP_COLUMNS: &str = r#"
    g.id, g.realm_id, g.name, g.description, g.parent_id, g.created_at, g.updated_at
"#;
...
/// PostgreSQL implementation of the User-Group assignment repository.
...
impl UserGroupRepo {
    ...
    pub async fn assign(
        ...
        let sql = "INSERT INTO user_groups (user_id, group_id) VALUES ($1, $2)";
        conn.execute_params(sql, &[&user_id, &group_id])
            .await
    ...
    pub async fn unassign(
        ...
        let sql = "DELETE FROM user_groups WHERE user_id = $1 AND group_id = $2";
        conn.execute_params(sql, &[&user_id, &group_id])
            .await
            .map_err(mapper::pg_err)?;
        Ok(())
    ...
    pub async fn find_groups_by_user(
        &self,
        conn: &mut Connection,
        user_id: Uuid,
        ...
        result
            .into_rows()
            .iter()
            .map(|r| Self::map_group_row(r))
            .collect::<Result<Vec<_>, _>>()
    }

    /// Find all users that belong to a specific group.
    pub async fn find_users_by_group(
        &self,
        conn: &mut Connection,
        group_id: Uuid,
    ...
    /// Find all roles assigned to a group (through group_roles junction).
    ...
    fn map_group_row(row: &wasi_pg_client::Row) -> Result<Group, OidcError> {
        Ok(Group {
