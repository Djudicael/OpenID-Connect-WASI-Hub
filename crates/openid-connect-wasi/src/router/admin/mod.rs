...
pub fn bad_request() -> Response {
    (StatusCode::BAD_REQUEST, Json(json!({"error": "bad_request"}))).into_response()
}
...
pub fn router() -> Router<AppState> {
    Router::new()
        ...
        .route("/api/users/{id}/roles/{role_id}", delete(users::unassign_role))
        ...
        .route("/api/users/{id}/groups/{group_id}", delete(users::unassign_group))
        .route("/api/groups/{id}/roles", get(audit::list_group_roles))
        .route("/api/groups/{id}/roles", post(audit::assign_role_to_group))
        .route("/api/groups/{id}/roles/{role_id}", delete(audit::unassign_role_from_group))
        .route("/api/users/{id}/account-recovery", post(users::initiate_account_recovery))
        .route("/api/users/{id}/impersonate", post(users::impersonate))
        .route("/api/maintenance/cleanup", post(audit::cleanup_expired))
        .route("/api/realms/{id}/password-policy", get(realms::get_password_policy))
        .route("/api/realms/{id}/password-policy", put(realms::update_password_policy))
        .route("/api/identity-providers", get(realms::list_identity_providers))
        .route("/api/identity-providers", post(realms::create_identity_provider))
        .route("/api/identity-providers/{id}", get(realms::get_identity_provider))
        .route("/api/identity-providers/{id}", put(realms::update_identity_provider))
        .route("/api/identity-providers/{id}", delete(realms::delete_identity_provider))
}
...
pub async fn connect(state: &AppState) -> Result<Connection, Response> {
    match wasi_pg_client::Connection::connect(&state.db_config).await {
        ...
        Err(e) => {
            ...
            Err((StatusCode::INTERNAL_SERVER_ERROR, Json(json!({"error": "internal"}))).into_response())
        }
    }
}
