...
pub async fn discovery_handler(state: OidcState) -> Json<Value> {
    ...
    let m = discovery_map! {
        ...
        "introspection_endpoint" => json!(format!("{issuer}/oidc/introspect")),
...
pub async fn realm_discovery_handler(state: OidcState, realm: String) -> Json<Value> {
    ...
    let m = discovery_map! {
        ...
        "jwks_uri" => json!(format!("{realm_base}/protocol/openid-connect/certs")),
        "introspection_endpoint" => json!(format!("{realm_base}/protocol/openid-connect/introspect")),
