...
const defaultConfig = {
  ldap: { base_dn: 'ou=people,dc=example,dc=com', user_filter: '(uid={identifier})', id_attribute: 'entryUUID', username_attribute: 'uid', email_attribute: 'mail', given_name_attribute: 'givenName', family_name_attribute: 'sn', group_attribute: 'memberOf', link_existing_users: false },
  active_directory: { base_dn: 'dc=example,dc=com', user_filter: '(|(userPrincipalName={identifier})(sAMAccountName={identifier}))', id_attribute: 'objectGUID', username_attribute: 'sAMAccountName', email_attribute: 'mail', given_name_attribute: 'givenName', family_name_attribute: 'sn', group_attribute: 'memberOf', link_existing_users: false },
...
class UserFederationPage extends BaseComponent {
  constructor() {
    super();
    this._state = { realms: [], realmId: '', providers: [], loading: true, showModal: false, editing: null, name: '', type: 'ldap', gatewayUrl: '', gatewaySecret: '', priority: 0, enabled: true, importUsers: true, syncGroups: true, config: JSON.stringify(defaultConfig.ldap, null, 2), workingId: '' };
  }
  ...
  async _save() {
    ...
    if (!body.name || !body.gateway_url || (!this._state.editing && !body.gateway_secret)) return showToast('Name, gateway URL, and secret are required', 'error');
  ...
  template() {
    ...
    return html`<c-page-layout title="User Federation"><div slot="actions"><c-button variant="primary" @click=${()=>this._open()}>+ Add provider</c-button></div><section data-doc-section="user-federation"><p style="color:var(--color-text-muted)">Connect realm users to LDAP, Active Directory, or Kerberos through a federation gateway.</p><div class="toolbar"><label>Realm: <select class="realm-select" .value=${s.realmId} @change=${event=>this._changeRealm(event)}>${s.realms.map(realm=>html`<option value=${realm.id}>${realm.display_name||realm.name}</option>`)}</select></label></div>${s.loading ? html`<div class="empty-state">Loading...</div>` : s.providers.length ? html`<c-table .columns=${columns} .rows=${s.providers}></c-table>` : html`<div class="empty-state"><div class="empty-state-text">No user federation providers configured</div></div>`}</section></c-page-layout>
    <c-modal title=${s.editing?'Edit federation provider':'Add federation provider'} @close=${()=>this._close()}>${s.showModal ? html`<div class="form"><div class="field"><label class="field-label">Name *</label><input class="field-input" .value=${s.name} @input=${event=>this.setState({name:event.target.value})}></div><div class="field"><label class="field-label">Directory type</label><select class="field-select" .value=${s.type} @change=${event=>{const type=event.target.value;this.setState({type,config:JSON.stringify(defaultConfig[type],null,2)});}}>${Object.entries(typeLabels).map(([value,label])=>html`<option value=${value}>${label}</option>`)}</select></div><div class="field"><label class="field-label">Federation gateway URL *</label><input class="field-input" type="url" placeholder="https://federation.example.com" .value=${s.gatewayUrl} @input=${event=>this.setState({gatewayUrl:event.target.value})}></div><div class="field"><label class="field-label">Gateway secret ${s.editing?'':'*'}</label><input class="field-input" type="password" .value=${s.gatewaySecret} @input=${event=>this.setState({gatewaySecret:event.target.value})}><div class="hint">Leave empty while editing to keep the current secret.</div></div><div class="field"><label class="field-label">Priority</label><input class="field-input" type="number" .value=${s.priority} @input=${event=>this.setState({priority:event.target.value})}></div><div class="field"><label class="field-label">Directory configuration</label><textarea class="field-input" rows="10" .value=${s.config} @input=${event=>this.setState({config:event.target.value})}></textarea></div><label class="field-checkbox"><input type="checkbox" ?checked=${s.enabled} @change=${event=>this.setState({enabled:event.target.checked})}> Enabled</label><label class="field-checkbox"><input type="checkbox" ?checked=${s.importUsers} @change=${event=>this.setState({importUsers:event.target.checked})}> Import users</label><label class="field-checkbox"><input type="checkbox" ?checked=${s.syncGroups} @change=${event=>this.setState({syncGroups:event.target.checked})}> Synchronize groups</label></div>` : ''}<div slot="footer"><c-button variant="secondary" @click=${()=>this._close()}>Cancel</c-button><c-button variant="primary" @click=${()=>this._save()}>Save</c-button></div></c-modal>`;
  }
}
